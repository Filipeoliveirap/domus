...
export function useDesmarcarTodosPresentes(eventoId: string) {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: () => inscricoesService.desmarcarTodosPresentes(eventoId),
    onMutate: async () => {
    ...
    onError: (error: unknown, _vars, contexto) => {
      ...
      notificar.erro('Não foi possível desmarcar presença', mensagem ?? 'Tente novamente.')
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['relatorio-evento', eventoId] })
      notificar.sucesso('Presença desmarcada para todos os inscritos.')
    },
  })
}
