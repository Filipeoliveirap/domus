...
/**
 * Harness de autorização por endpoint (ver AutenticacaoTestSupport) aplicado à tela de
 * início. Sem restrição de perfil — qualquer autenticado vê seu próprio resumo.
 */
@SpringBootTest
@AutoConfigureMockMvc
@Transactional
class InicioControllerTest implements PostgresTestContainerSupport {
    ...
    void setup() {
        ...
        igreja = igrejaRepository.save(Igreja.builder()
                .nome("Igreja Teste Inicio " + UUID.randomUUID())
                .emailContato("inicio-" + UUID.randomUUID() + "@teste.com")
                .build());
        entityManager.flush();
    }
    ...
    @Test
    void inicio_semAutenticacao_recusaCom401() throws Exception {
        mockMvc.perform(get("/inicio"))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void inicio_acessoComum_permitidoCom200() throws Exception {
        Usuario comum = usuarioComRole("ACESSO_COMUM");

        mockMvc.perform(auth.autenticado(get("/inicio"), comum))
