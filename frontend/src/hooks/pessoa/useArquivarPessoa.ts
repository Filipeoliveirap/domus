...
export function useArquivarPessoa(pessoa: PessoaResponse, onClose: () => void) {
  ...
  const confirmar = async () => {
    ...
    setIsLoading(true)
    try {
      await pessoasService.arquivar(pessoa.id)
      invalidarCache(queryClient, 'pessoa')
      notificar.sucesso(`${pessoa.nome} foi arquivado.`)
      onClose()
    } catch (error: unknown) {
