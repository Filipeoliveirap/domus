...
@Transactional
class PostagemControllerTest implements PostgresTestContainerSupport {
    ...
    void setup() {
        ...
        igreja = igrejaRepository.save(Igreja.builder()
                .nome("Igreja Teste Postagem " + UUID.randomUUID())
                .emailContato("postagem-" + UUID.randomUUID() + "@teste.com")
                .build());
        entityManager.flush();
    }
    ...
    @Test
    void listarMural_semAutenticacao_recusaCom401() throws Exception {
        mockMvc.perform(get("/postagens/mural"))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void listarMural_autenticado_permitidoCom200() throws Exception {
        Usuario comum = usuarioComRole("ACESSO_COMUM");
        mockMvc.perform(auth.autenticado(get("/postagens/mural"), comum))
                .andExpect(status().isOk());
    }

    @Test
    void listarFeed_autenticado_permitidoCom200() throws Exception {
        Usuario comum = usuarioComRole("ACESSO_COMUM");
