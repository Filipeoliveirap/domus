...
export function useMaiorLancamento(periodo: PeriodoRelatorio, enabled = true, igrejaId?: string, vinculo?: Vinculo | '') {
  return useQuery({
    queryKey: ['relatorios', 'maior-lancamento', periodo, igrejaId ?? 'minha-igreja', vinculo || 'todos'],
    queryFn: () => relatorioService.maiorLancamento(periodo, igrejaId, vinculo),
    enabled,
  })
}