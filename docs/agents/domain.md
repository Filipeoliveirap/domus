...
O contexto principal do projeto já vive no `CLAUDE.md` (roadmap, convenções,
modelo de dados, princípios). **Não duplicar** — `CONTEXT.md` aponta pra lá.
...
tomar a decisão errada de novo daqui a 6 meses". Path: `docs/adr/`.

## Consumer rules para skills vizinhas

- Antes de criar contexto de domínio novo, **checar `CLAUDE.md`** — o que
  você vai escrever já está lá?
- Antes de criar ADR novo, **checar `docs/adr/`** — já existe decisão sobre o
  mesmo assunto?
- `CONTEXT.md` é índice, não conteúdo. Se crescer demais, é sinal de que algo
  deveria voltar pro `CLAUDE.md` ou virar ADR.
