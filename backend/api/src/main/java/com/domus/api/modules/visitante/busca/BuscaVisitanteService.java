...
@Slf4j
public class BuscaVisitanteService {
    ...
    public List<ResultadoBusca> buscar(String termo, UUID igrejaId, int limite) {
        ...
        SearchHits<VisitanteDocument> hits = elasticsearchOperations.search(nativeQuery, VisitanteDocument.class);

        return hits.stream()
                .map(hit -> {
                    VisitanteDocument doc = hit.getContent();
                    return new ResultadoBusca(
                            doc.getId(),
                            TipoEntidadeOutbox.VISITANTE,
                            doc.getNome(),
                            doc.getTelefone(),
                            doc.getCelulaId()
                    );
