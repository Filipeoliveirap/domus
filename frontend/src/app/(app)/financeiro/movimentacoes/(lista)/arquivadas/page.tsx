...
export default function MovimentacoesArquivadasPage() {
  ...
  return (
    <>
      <div className={styles.lista}>
        {movimentacoes.map((m) => (
          <div key={m.id} className={styles.linha}>
...
function ModalExcluirDefinitivo({ movimentacao, onClose }: { movimentacao: MovimentacaoArquivadaResponse; onClose: () => void }) {
  ...
  const consequencias: Consequencia[] = movimentacao.temContribuinte
    ? [{
        tipo: 'perde',
  ...
  return (
    <ModalConfirmacaoCritica
      ...
      mensagem={
        <>
          Isso vai apagar <strong>{identificacao}</strong> de vez, inclusive do relatório.
          Não tem como desfazer.
        </>
      }
      consequencias={consequencias}
      palavraConfirmacao={identificacao}
